...
export async function showDashboard(ctx: any, getRun: () => SwarmRun | undefined): Promise<DashboardAction> {
  if (ctx.mode !== "tui") {
...
export function widgetLines(run?: SwarmRun): string[] | undefined {
  ...
  return [`◐ swarm ${done}/${run.plan?.subtasks.length ?? 0} · ${active ? `${active.subtaskId} ${active.currentAction}` : run.phase} · $${run.totals.cost.toFixed(3)}${awaiting ? ` · ⚠${awaiting}` : ""}`];
}

class DashboardComponent {
  private selected = 0;
  private readonly theme: Theme;
  private readonly getRun: () => SwarmRun | undefined;
  private readonly done: (action: DashboardAction) => void;
  private readonly rerender: () => void;

  constructor(theme: Theme, getRun: () => SwarmRun | undefined, done: (action: DashboardAction) => void, rerender: () => void) {
    this.theme = theme;
    this.getRun = getRun;
    this.done = done;
  ...
  render(width: number): string[] {
    ...
    if (!run) return ["No active swarm run"];
    const w = Math.max(70, Math.min(width, 120));
    const inner = w - 2;
    const row = (value: string) => `${this.theme.fg("border", "│")}${pad(value, inner)}${this.theme.fg("border", "│")}`;
    const lines = [this.theme.fg("border", `╭${"─".repeat(inner)}╮`), row(` ${this.theme.fg("accent", `SWARM · ${run.plan?.taskSummary ?? run.task}`)} · ${run.phase} · $${run.totals.cost.toFixed(3)}`)];
    const workers = Object.values(run.workers);
