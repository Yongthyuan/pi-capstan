...
export function assessPiCompatibility(version: string, api: PiCapabilities): { level: CompatibilityLevel; message: string } {
  ...
  if (major === 0 && minor === 84 && patch >= 1) return { level: "compatible", message: `Pi ${version} 满足 0.84.x 能力契约，但不是已验证构建` };
  return { level: "unsupported", message: `Pi ${version} 超出支持范围 >=0.84.1 <0.85.0` };
}
